...
int i, pardd_start = 1;
	...
	if (width > length)
	{
		...
		{
			if (extra_c)
				buffer[--pardd_start] = extra_c;
			return (write(1, &buffer[pardd_start], i - pardd_start) +
			write(1, &buffer[ind], length - (1 - pardd_start)));
		}
	}
	if (extra_c)
		buffer[--ind] = extra_c;
